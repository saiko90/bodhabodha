...
export default function ResultClient({ result, products }: ResultClientProps) {
  ...
  // --- ECRAN DE RÉSULTAT ---
  return (
    <main className="relative min-h-screen text-white flex flex-col items-center justify-start overflow-x-hidden bg-black animate-in fade-in duration-1000">
      
      {/* 1. NAVIGATION (AJOUTÉ ICI) */}
      <nav className="absolute top-0 w-full p-6 md:p-8 flex justify-between items-center z-50 animate-in slide-in-from-top-5 duration-1000 delay-500">
        <Link href="/" className="text-xl md:text-2xl font-serif font-bold tracking-tight text-white hover:text-gold-300 transition-colors drop-shadow-md">
          BodhaBodha
        </Link>
        <Link 
            href="/" 
            className="text-[10px] md:text-xs uppercase tracking-[0.2em] text-white/60 hover:text-gold-300 border border-white/10 hover:border-gold-500/50 px-4 py-2 rounded-full transition-all backdrop-blur-sm"
        >
          Retake Assessment
        </Link>
      </nav>
      ...
      {/* 3. CONTENU PRINCIPAL */}
      <div className="relative z-10 max-w-5xl w-full mt-24 mb-20 px-4"> 
